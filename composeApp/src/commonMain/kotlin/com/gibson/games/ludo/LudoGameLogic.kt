...
data class BoardState(
    ...
    val availableMoves: List<Int> = emptyList() // New: Stores available dice values for movement
)
...
fun handleTurn(boardState: BoardState, rules: GameRules, diceRoll: DiceRoll): BoardState {
    ...
    // Determine available moves based on the rolled dice
    ...
    if (diceRoll.die1 != 6 && diceRoll.die2 != 6) {
        availableMoves.add(diceRoll.die1)
        availableMoves.add(diceRoll.die2)
        availableMoves.add(diceRoll.total)
    } else if (diceRoll.die1 == 6 && diceRoll.die2 != 6) {
        availableMoves.add(diceRoll.die2)
    } else if (diceRoll.die1 != 6 && diceRoll.die2 == 6) {
        availableMoves.add(diceRoll.die1)
    }
    ...
    return boardState.copy(
        ...
        gamePhase = if (winner != null) GamePhase.GAME_OVER else GamePhase.MOVING, // Stay in MOVING phase to allow selection
        winner = winner,
        ...
        availableMoves = availableMoves.distinct() // Ensure unique values
    )
}
    ...
    return player.tokens.filter { token ->
        when {
            token.position == -1 -> {
                ...
                !rules.requiresSixToExitBase || diceValue == 6
            }
            token.position in 0..51 -> {
                // Token on main path - can always move if not going past finish
                token.position + diceValue <= 57 // 52-57 is the home stretch
            }
            token.position in 100..105 -> {
                // Token in home path - can move if not going past finish
                (token.position - 100) + diceValue <= 6
            }
            token.position == 200 -> {
